...
`src/lib/legacy/etf/**`와 `test/legacy/**`는 v1.7 이전 Gate·키움 선정 계약을 보존하는 역사적 증거 경로다. 이 경로의 코드는 현재 v1.7 도메인 경로(`src/lib/etf/**`)와 CI 필수 Gate로 import하지 않으며, 레거시 동작 확인이 필요할 때만 `npm run test:legacy`로 수동 실행한다.
