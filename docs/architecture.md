...
1. DG1에서 승인된 한국·미국 시장별 데이터 소스의 사용 권한과 기준일 재현 가능성을 확인합니다. 확인 실패 시 UI 본개발을 중단하거나 통과 시장만 `Conditional Go`로 종단 구현합니다.
...
| 경계             | 책임                                                    | 핵심 기록                                           |
| ---------------- | ------------------------------------------------------- | --------------------------------------------------- |
| 데이터 소스·입력 | 시장별 권한, 출처, 갱신 주기, CSV 대체 경로 관리        | 소스 메타데이터, 원본 as-of 스냅샷                  |
...
- 목표 범위는 사전 필터를 통과한 한국·미국 ETF입니다. DG1은 각 시장 5개 이상, DG2는 각 20~30개로 단계 확장하며, 시장별 데이터·권한 실패 시 해당 시장을 `Conditional Go` 또는 `No-Go`로 분리합니다.
...
- 소스 사용 권한과 재배포 가능성은 API 존재 여부와 별개입니다. DG1에서 확인하지 못하면 해당 시장의 자동화와 웹 확장을 진행하지 않습니다.
...
1. `docs/PRD.md`의 DG1 기준으로 한국·미국 시장별 권한, 기준일, 필수 필드, CSV 대체 가능성을 판정합니다.
