...
> **상태 (2026-09-15):** 통과 — DG0 범위 한정 Go 판정<br>
> **Owner:** kwh8121<br>
> **Approver:** kwh8121<br>
> **판정:** Go (2026-09-15, Approver: kwh8121) — DG0 범위에 한정한다.

## DG0 Gate 판정 기록 (2026-09-15)

Approver `kwh8121`은 정합화 증거를 근거로 DG0 범위에 한정하여 `Go`를 판정했다. 이 판정은 KRX 실제 수집, 필드 의미 승인, 과거 재현 또는 후속 DG의 통과를 뜻하지 않는다. 실제 Supabase 프로젝트·migration 적용 상태는 이 Gate에서 조회되지 않았으며, DG1의 자료 검사와 새 migration 계획에서 읽기 전용으로 확인한다.
...
| 2 헌법 VII.5            | 2A' 단계별 GREEN 유지 및 #6 분리 | `9861db4` 및 2026-09-15 kwh8121 작업 세션 구두 승인(공개 기록 없음): #6-A 현재 master 차단과 #6-B 미래 가격·상태 차단을 모두 P0-02 완료 조건의 최초 RED→GREEN으로 두고, DG2.5에서는 과거 재현 Gate 차원의 재검증을 수행한다고 헌법에 반영했다. |
...
| 현재 master/환율·분배 계약 | `src/lib/legacy/etf/collection.ts`, `test/legacy/etf-collection.test.ts`                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                      |
...
| 키움 선정 Gate 계약        | `src/lib/legacy/etf/gate-validation.ts`, `test/legacy/gate-validation.test.ts`, `test/legacy/legacy-gate-status.test.ts`                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                      |
| v1.7 상태 검사             | `test/project-status.test.ts`                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 |
...
| ------------------------------------ | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------- | -------------------------------------------------------------------------- |
| 국내 가격·거래량·거래대금 정본이 KRX | PRD·현행 architecture는 KRX 정본을 선언한다. `src/lib/etf/selection-source.ts`는 v1.7 선정 원천을 `krx`로 선언한다. 기존 `krx-dg2-adapter.ts`는 이전 전략/evidence 계약에 결합돼 직접 재사용할 수 없다.                                         | Go (DG0 범위) | DG1에서 독립 KRX 수집 계약을 RED→GREEN으로 재증명한다.                     |
| 문서·코드의 선정 원천이 KRX로 일치   | `src/lib/etf/selection-source.ts`(`SELECTION_SOURCE = 'krx'`), `test/v17-selection-source-contract.test.ts`가 `check-all`에서 v1.7 경로의 레거시 선정 표식·legacy import·CI 강제를 검사한다. 키움 선정 계약은 `src/lib/legacy/etf/`로 격리했다. | Go (DG0 범위) | 가드 테스트는 DG0 계약 정합화 증거이며, 실제 수집 검증은 DG1에서 수행한다. |
| 기존 충돌 계약 폐기                  | 키움 선정 계약은 코드 격리, CI 단계 제거, `status:check` 분리 완료. 환율·분배금·리서치·포트폴리오·자동 평가·기존 UI는 결정 1-B1에 따라 v1.7 완료 증거에서 제외하고 해당 DG에서 교체한다. migration 적용 여부는 Owner 읽기 전용 조회가 필요하다. | Go (DG0 범위) | 실제 Supabase 적용 상태는 DG1 자료 검사와 새 migration 계획에서 확인한다.  |
...
2. 헌법 VII.5는 단계별 GREEN 유지로 개정한다. 2026-09-15 kwh8121의 작업 세션 구두 승인(공개 기록 없음)에 따라 #6-A 현재 master 차단과 #6-B 미래 가격·상태 차단은 모두 P0-02 완료 조건에서 최초 GREEN으로 증명하고, DG2.5에서는 과거 재현 Gate 차원의 미래 접근 차단을 재검증한다.
...
이 판정은 DG0 계약 통일에만 적용한다. DG1 이상은 각 선행 조건과 사람 Gate 판정을 별도로 충족해야 하며, 기존 완료 표기를 승계하지 않는다.
