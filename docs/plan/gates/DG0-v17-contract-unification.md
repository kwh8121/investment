...
> **상태 (2026-09-15):** DG0 판정 대기 — 정합화 증거 제출, Gate 판정 없음<br>
> **Owner:** kwh8121<br>
> **Approver:** kwh8121<br>
...
| ------------------------------------ | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | --------------------- | ----------------------------------------------------------------------- |
| 국내 가격·거래량·거래대금 정본이 KRX | PRD·현행 architecture는 KRX 정본을 선언한다. `src/lib/etf/selection-source.ts`는 v1.7 선정 원천을 `krx`로 선언한다. 기존 `krx-dg2-adapter.ts`는 이전 전략/evidence 계약에 결합돼 직접 재사용할 수 없다.                                         | 증거 제출 — 판정 대기 | DG1에서 독립 KRX 수집 계약을 RED→GREEN으로 재증명하는 조건을 수용하는지 |
| 문서·코드의 선정 원천이 KRX로 일치   | `src/lib/etf/selection-source.ts`(`SELECTION_SOURCE = 'krx'`), `test/v17-selection-source-contract.test.ts`가 `check-all`에서 v1.7 경로의 레거시 선정 표식·legacy import·CI 강제를 검사한다. 키움 선정 계약은 `src/lib/legacy/etf/`로 격리했다. | 증거 제출 — 판정 대기 | 가드 테스트가 DG0 조건을 충분히 대표하는지                              |
| 기존 충돌 계약 폐기                  | 키움 선정 계약은 코드 격리, CI 단계 제거, `status:check` 분리 완료. 환율·분배금·리서치·포트폴리오·자동 평가·기존 UI는 결정 1-B1에 따라 v1.7 완료 증거에서 제외하고 해당 DG에서 교체한다. migration 적용 여부는 Owner 읽기 전용 조회가 필요하다. | 증거 제출 — 판정 대기 | 문서 처분과 legacy 격리만으로 DG0 시점의 충돌 계약 폐기를 인정하는지    |
