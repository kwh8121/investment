...
| 2    | 2A' 단계별 GREEN 유지           | §12.2 테스트는 해당 단계 착수 시 RED로 도입하고 GREEN 이후 유지한다. #6은 #6-A 현재 master 차단과 #6-B 미래 가격·상태 차단으로 나누되 모두 P0-02 완료 조건에서 최초 GREEN으로 증명하고, DG2.5에서 과거 재현 Gate 차원으로 재검증한다. |
...
| #6-B 미래 가격·상태 차단 | 기준일 이후 가격·상태 접근 차단. 이후 가격은 평가 모듈만 읽음 | P0-02           |

3A' 실행 시 `specify init --force` 전후로 기존 추적 `.claude/skills/**`, `.claude/agents/**`, `.claude/commands/**`, `.claude/hooks/**`, `.claude/settings.local.json` 변경 여부를 확인한다. 변경이 발견되면 자동 복구하지 않고 중단하며, diff를 Gate 증거에 기록한다.
...
| 6   | `> **Last Amended:** [미정]`                  | `> **Last Amended:** <실행일 YYYY-MM-DD> — VII.5 단계별 GREEN 유지로 개정, #6-B를 P0-02 완료 조건으로 정합화, Gate 검증 도구 역할 정정 (영향: 적용 대상, Art. III.6, Art. VII.5, Development Workflow; 근거: PRD §12.2·§14·P0-02, 2026-09-15 kwh8121 승인, Spec Kit 1.0.6 도구 동작)` |
...
```text
*   **VII.5** §12.2 필수 회귀 테스트는 PRD §14 순서상 해당 단계 착수 시 RED로 도입하고, GREEN이 된 이후의 모든 머지에서 GREEN을 유지한다. DG2.5 판정 전까지 14개 모두 GREEN이어야 한다. #6은 #6-A 현재 마스터 차단과 #6-B 미래 가격·상태 차단으로 나누며, 두 조각 모두 P0-02 완료 조건에서 최초 GREEN으로 증명한다. DG2.5는 #6의 GREEN 유지와 과거 재현 Gate 차원의 미래 접근 차단을 재검증한다. 테스트 삭제·완화는 헌법 개정 사안이다.
```
...
5. §12.2 추적 표의 #6 행 다음 증거 위치를 `헌법 VII.5(0.2.0): #6-A 현재 master 차단과 #6-B 미래 가격·상태 차단은 P0-02에서 최초 RED→GREEN, DG2.5에서 재검증`으로, #1~5·#14 행과 #7~13 행을 `헌법 VII.5(0.2.0) 단계별 도입 — ROADMAP 순차 작업 표의 단계`로 바꾼다.
