...
> 제품 요구사항 권위 문서: `PRD.md`  
> 원천 실행 계획: `plan/ETF_MVP_이후_진행_흐름_v0.1.md`  
> Gate 증거 위치: `plan/gates/DG{번호}-{주제}.md`

## 1. 목적

이 문서는 `PRD.md`의 P0-01~P0-10을 구현·검증하기 위한 **실행 순서·일정·완료 증거의 권위 문서**다. 원천 실행 계획을 Task와 Gate로 구체화하며, 의존성, 마일스톤, Go/No-Go Gate와 완료 증거를 정의한다. 일정은 Gate 통과를 전제로 하며, 주차가 지났다는 이유만으로 다음 단계에 진입하지 않는다.
...
| Gate                    | 목표 시점    | 필수 증거                                                      | Owner (담당) | Approver (승인) | 판정                        |
| ----------------------- | ------------ | -------------------------------------------------------------- | ------------ | --------------- | --------------------------- |
| DG0 범위                | 착수 전      | PRD v1.0, 목표 시장, 비목표, Open Questions                    | 제품 책임자  | 검토자          | Go / 보완                   |
| DG1 데이터·권한         | W1           | 한·미 각 5개, 필드 매핑, 권한표, 원본·정규화, 오류·재시도 로그 | 제품 책임자  | 검토자          | Go / Conditional Go / No-Go |
| DG2 전략 방향성         | W3           | 각 20~30개 간이 백테스트, 전략 구분, 컷·필터 민감도            | 제품 책임자  | 검토자          | Go / 재설계                 |
| DG3 전략·위험           | W6           | 3~5년 롤링 백테스트, 하락장, 비용·위험·교체 분석               | 제품 책임자  | 검토자          | Go / 재설계                 |
| DG4 제품 알파           | W8           | P0 종단 흐름, 재현성, 발행 차단, 복구, 내부 QA                 | 제품 책임자  | 검토자          | Go / Conditional Go / No-Go |
| DG5 소액 실전 진입 판단 | 운영 검증 후 | 8~12주 안정성, 세후·비용 재계산, 6개월 전향검증 계획           | 제품 책임자  | 검토자          | 별도 승인                   |

모든 Gate 문서는 `docs/plan/gates/` 아래에 `DG{번호}-{주제}.md` 형식으로 저장하며 판정일, 판정자(Approver), 담당자(Owner), 근거, 미충족 항목, 예외 만료일과 재검토 날짜를 포함한다. Owner는 Gate 증거를 생성·제출하는 역할이고, Approver는 제출된 증거를 검토해 Go/Conditional Go/No-Go를 확정하는 역할이다. 두 역할은 초기에 동일인이 겸임할 수 있으나 문서상 역할은 항상 분리해 기록하며, `PRD.md` 11장과 필드를 일치시킨다.

> **일정 주석 — 원천 계획 대비 변경:** 원천 실행 계획(`plan/ETF_MVP_이후_진행_흐름_v0.1.md`)이 가정한 DG2 W2 시점은 Task 004→005→006→007→008의 순차 의존성상 달성할 수 없다. 본 로드맵은 DG2를 W3로 조정하며, 이 문서의 일정이 원천 계획의 오래된 W2 가정을 대체하는 현재 권위 기준이다.
...
**상태:** 완료 (Completed) — 2026-08-25, DG0 Go 판정
(`docs/plan/gates/DG0-scope-baseline.md`)  
**의존성:** 없음  
**완료 Gate:** DG0

- `PRD.md` v1.0의 목표 시장, 전략, 사용자, 비목표와 P0-01~P0-10 검토
...
- `architecture.md`를 승인된 범위와 기술 선택으로 갱신
...
- 한국·미국 각 20~30개 또는 DG1 승인 범위로 실행
...
- 기준 총 투자자금 1,000만원, 총 투자 비중 90% 이하와 현금 10% 이상 검증
- 분할매수, ETF별 8%와 포트폴리오 -8/-10/-15/-20% 규칙 검증
- 미국 ETF 60%, ETF당 30% 제한 검증
...
**의존성:** DG4 Go 또는 승인된 Conditional Go
...
| W3   | 전략별 스코어카드, 소규모 유니버스 간이 백테스트        | 각 20~30개 방향성 검증                                            | 007, 008      | DG2      |
...
각 Task가 직접 충족시키는 AC ID를 표시한다. AC 정의와 Evidence는 `PRD.md` 16.2절을 참조한다.
...
모든 Task 001~014와 모든 P0-01~P0-10 AC(총 50개)가 위 표를 통해 상호 추적 가능하다.
...
**Task 001이 완료되었으므로 Task 002와 Task 003으로 DG1 — Kiwoom API Reality Check를 수행한다.**

첫 주 필수 산출물:

1. 한국·미국 ETF 각 5개 이상의 실데이터 수집 결과
2. API별 인증 헤더·키·토큰·쿠키·계정 식별자를 제거한 요청·응답 샘플·작업 로그와 필드 매핑표
3. 연속조회, 수정주가, 환율과 분배금 검증 결과
4. 데이터 소스별 권한·허용 용도 매트릭스
5. 오류, 재시도, 결측과 보조 소스 대조 결과
6. 시장별 `Go / Conditional Go / No-Go` 판정
7. PRD Open Questions의 확정·보류·변경 목록

DG1 통과 전에는 Supabase 스키마나 전체 UI 백로그를 확정하지 않는다.

---

**진행 상황:** 1/14 Tasks 완료 (7%)  
**현재 Phase:** Phase 0 — 기준선과 Reality Check  
**다음 Gate:** DG1 데이터·권한 Reality Check
