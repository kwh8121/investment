...
| DG1 데이터·권한         | W1           | 한·미 각 5개, 필드 매핑, 권한표, 원본·정규화, 오류·재시도 로그 | 제품 책임자  | 검토자          |
| DG2 전략 방향성         | W3           | 각 20~30개 간이 백테스트, 전략 구분, 컷·필터 민감도            | 제품 책임자  | 검토자          |
...
모든 Gate 문서는 `docs/plan/gates/` 아래에 `DG{번호}-{주제}.md` 형식으로 저장한다. 판정 기록 필드와 Owner/Approver 역할 정의는 `PRD.md` 11장을 따른다.
...
모든 Task 001~014와 `PRD.md` 16.2절의 모든 P0-01~P0-10 AC가 위 표를 통해 상호 추적 가능하다.
