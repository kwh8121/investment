...
- 검사: `src/lib/etf/**`, `src/lib/research/**`, `src/app/**`, `src/components/**`, `test/**`, `tests/e2e/**`, `migrations/**`, `scripts/**`, 이전 `docs/plan/gates/**`
...
- [x] **1단계: 감사 대상 구현 인벤토리 수집**

실행: `rg --files src/lib/etf src/lib/research src/app src/components test tests/e2e migrations scripts docs/plan/gates | sort`
...
- PRD §11 DG0은 자산 매트릭스와 사람 판정 패킷으로 **부분적으로** 다룬다. KRX 선정 원천 교체·격리와 기존 충돌 계약의 처리에는 사람 결정이 남아 있다.
- PRD §2, §3, §4, §9, §12.2, §15의 감사·분석 위치는 기록했으나, 후속 DG의 RED→GREEN 증거와 사람 Gate 판정은 아직 없다.
