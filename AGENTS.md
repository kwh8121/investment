...
| DG 수준의 실행 순서 / 의존관계 / 프로그램 기준선 | `docs/ROADMAP-v1.7.md` | Linear·계획·Gate 문서에는 DG 링크와 최소 상태만 둠 |
| 진행 중 개별 작업 / 담당 / 블로커 / 검토 요청 / 상태 전이 | Linear | 정본 문서·PR·Gate 링크와 짧은 결과만 둠 |
...
- 계획 초안·검증 요청·진행 중 개별 작업 상태는 Linear에 둔다. `docs/ROADMAP-v1.7.md`는 DG 수준의 승인된 프로그램 기준선만 유지한다. 승인된 구현 계획만 `docs/plan/`에 불변 스냅샷으로 남긴다.
- `docs/superpowers/plans/`는 Superpowers가 생성한 실행 계획의 작업 공간이다. 승인된 계획을 중복 복사하지 말고, 정본 계획의 링크·상태·차이만 기록한다.
- `docs/ROADMAP-v1.7.md`는 DG 수준의 일정·의존성·프로그램 기준선의 정본이며 개별 Linear 이슈의 실시간 상태, Gate 판정 근거나 제품 요구사항 본문을 복제하지 않는다. `docs/ROADMAP.md`는 이전 제품 방향의 역사적 스냅샷이다.
